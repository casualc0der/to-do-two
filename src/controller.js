...
const controller = (() => {
  ...
  const addTask = () => {
    ...
    retrieveDB
      ...
      .todoListCollection.push(model.todoList(uuidv4(), task));
  ...
  const deleteTask = (dbId, Tid) => {
    ...
    index.filter(e => {
      if (e.id === Tid) {
        ...
        retrieveDB
          .filter(e => e.id === dbId)[0]
          .todoListCollection.splice(i, 1);
  ...
  const editTodo = (dbId, Tid, title, description, datedue, priority) => {
    ...
    index.filter(e => {
      if (e.id === Tid) {
        const i = index.indexOf(e);
        retrieveDB.filter(e => e.id === dbId)[0].todoListCollection[
          i
        ].title = title;
        retrieveDB.filter(e => e.id === dbId)[0].todoListCollection[
          i
  ...
  const retrieveTodo = (dbId, Tid) => {
    ...
    const QUERY = retrieveDB
      .filter(e => e.id === dbId)[0]
      .todoListCollection.filter(e => e.id === Tid)[0];
    return {
      title: QUERY.title,
      description: QUERY.description,
      dueDate: QUERY.datedue,
